...
- **Upstream, merged by the maintainers:** two Windows long-path fixes in the client library the Hugging Face stack is built on, a deep-path download crash in [huggingface_hub #4546](https://github.com/huggingface/huggingface_hub/pull/4546) and a silently disabled tree cache in [huggingface_hub #4896](https://github.com/huggingface/huggingface_hub/pull/4896); a corrected patched-version range in [github/advisory-database #8824](https://github.com/github/advisory-database/pull/8824); truecopy listed under Security in [awesome-mcp-servers #10332](https://github.com/punkpeye/awesome-mcp-servers/pull/10332); an always-out-of-range index that gosec's G602 analyzer waved through under an equality guard, in [securego/gosec #1746](https://github.com/securego/gosec/pull/1746); conditional-formatting and data-validation ranges that moved onto the wrong row when their first row was deleted, in [qax-os/excelize #2402](https://github.com/qax-os/excelize/pull/2402); task lines with a one- or two-character body, or a body opening with a markdown link or an attribute, that never received the page reference their Linked Mentions toggle needs, in [silverbulletmd/silverbullet #2128](https://github.com/silverbulletmd/silverbullet/pull/2128); a numeric equality threshold that excluded a difference exactly at the threshold, unlike the point comparison beside it, in [williamngan/pts #228](https://github.com/williamngan/pts/pull/228); an NBA scoreboard command that asked ESPN for a hyphenated date range the endpoint rejects with 400, replaced with one request per day, in [raycast/extensions #31314](https://github.com/raycast/extensions/pull/31314); a WebSocket parser that kept a message's compressed flag after the message completed, so a stray continuation frame was accepted instead of failing the connection, in [nodejs/undici #5827](https://github.com/nodejs/undici/pull/5827); a consensus-layer TTL cache that expires entries on read instead of running a sweep goroutine, and a peer-refresh loop that ran on a background context after its owner closed, in [erigontech/erigon #24022](https://github.com/erigontech/erigon/pull/24022).
