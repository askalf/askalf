...
When Scout finds a security vulnerability, Aegis doesn't know until its next scheduled cycle hours later. When Builder changes a database schema, Reviewer doesn't know until it happens to run. Cross-domain work that should trigger collaboration instead disappears into execution logs.
