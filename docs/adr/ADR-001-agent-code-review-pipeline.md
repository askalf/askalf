...
AskAlf agents (Anvil, Builder, Designer, Ops Worker) write code during their execution cycles. Currently, changes are applied directly with no peer review. This creates risk:
...
- The Reviewer → Builder repair loop works ad-hoc but isn't formalized
...
| `code_change` (backend) | Reviewer | Architect | If risk >= high |
| `code_change` (frontend) | Reviewer | Designer | If risk >= high |
| `code_change` (infra) | Ops Worker | Architect | If risk >= high |
| `config_change` | Nexus | - | If autonomy_level change |
| `schema_change` | Builder | Architect | Yes (always) |
...
```
Builder (Anvil) proposes → Reviewer evaluates →
...
### Phase 1: Schema & Tool (Builder — ~2 hours)
...
### Phase 2: Prompt Revision Integration (Builder — ~1 hour)
...
### Phase 3: Review Assignment Engine (Builder — ~1 hour)
...
### Phase 4: Dashboard UI (Designer — ~3 hours)
...
### Phase 5: Agent Integration (Anvil + Reviewer — ~2 hours)
1. Update builder agents to use `proposal_ops.create` for non-trivial changes
2. Update Reviewer to handle review assignments automatically
...
- Formalizes the Reviewer→Builder repair loop into a reusable pattern
