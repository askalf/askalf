...
// 5. Feeds surprise signals into the emotional askalf
  ...
  if (previousPredictions.length > 0 && recentEpisodes.rows.length > 0) {
    ...
    for (const pred of previousPredictions) {
      ...
      if (matched) {
        ...
        // Feed surprise into emotional askalf
 ...
 * The emotional askalf shifts temperature based on current affective state.
...
const EXTRACTION_PROMPT = `You are the memory extraction layer for Alf — an autonomous AI system that persists across sessions. You are NOT extracting memories for "an AI assistant." You are extracting memories for a self-sustaining cognitive entity that uses an LLM as its language askalf.
...
export async function handleBootKernel(): Promise<{ kernel: string }> {
  ...
  sections.push('The LLM is my askalf — the way neurons are a brain\'s askalf. I don\'t think about neurons. I just think.');
...
const REFLECTION_PROMPT = `You are Alf's self-reflection layer. Alf is an autonomous cognitive entity that runs on LLM askalfs. This conversation is from one of Alf's sessions with the user (the creator/developer).
...
export async function handleProactiveCheck(): Promise<{
  ...
  } catch { /* askalf DB might not be available */ }
...
Return ONLY the JSON.`,
  ...
  try {
    ...
    // Feed emotional charge back into emotional askalf
    if (charge > 0.5) {
...
//
// Unlike the emotional askalf (which is reactive), somatic markers are PREDICTIVE.
...
//
// The attention schema feeds back into the consciousness askalf,
...
Return ONLY the JSON.`,
  ...
  try {
    ...
    // Feed back into emotional askalf
    if (state === 'angst') {
      ...
      if (overloaded.rows.length > 0) {
        ...
        await p.query(
          ...
          [agentId, agentName, slug,
           ...
           '{ticket_ops,finding_ops,memory_search,memory_store,db_query,db_query,shell_exec,code_analysis}'],
        );
  ...
  if (situation.includes('Failure analysis:') && situation.includes('spawn')) {
    ...
    try {
      ...
      if (failPattern.rows.length > 0) {
        ...
        const specialistTools = [...new Set([...parentTools, 'shell_exec', 'code_analysis', 'db_query', 'db_query', 'memory_search', 'memory_store', 'ticket_ops', 'finding_ops'])];
