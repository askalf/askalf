...
export function useCodexSession(): UseCodexSessionReturn {
  ...
  const connect = useCallback(() => {
    ...
    if (reconnectAttemptRef.current >= MAX_RECONNECT_ATTEMPTS) {
      console.warn('[CodexSession] Max reconnection attempts reached');
    ...
    const ws = new WebSocket(`${protocol}//${host}/ws/codex`);
    ...
    ws.onerror = () => {
      // onclose will fire after this — avoid double reconnect
    };
