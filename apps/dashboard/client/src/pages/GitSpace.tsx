...
export default function GitSpace() {
  ...
  // Auto-switch to detail view when branch selected
  const handleBranchSelect = useCallback((branchName: string) => {
    setSelectedBranch(branchName);
    setViewMode('detail');
  }, [setSelectedBranch]);

  // Auto-open deploy after merge
  const showDeployPrompt = useMemo(() => {
    return merged && viewMode === 'detail';
  }, [merged, viewMode]);

  return (
    <div className={`cr-shell ${reviewOpen ? 'cr-panel-open' : ''}`}>
