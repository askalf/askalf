...
type SettingsTab = 'general' | 'api-keys' | 'costs' | 'integrations' | 'channels' | 'devices' | 'preferences' | 'infrastructure' | 'reports' | 'webhooks' | 'team' | 'migration';

const VALID_TABS: SettingsTab[] = ['general', 'api-keys', 'costs', 'integrations', 'channels', 'devices', 'preferences', 'infrastructure', 'reports', 'webhooks', 'team', 'migration'];
...
const NL_PATTERNS: Array<{ pattern: RegExp; tab: SettingsTab; message: string }> = [
  ...
  { pattern: /migrat|import/i, tab: 'migration', message: 'Opening Migration — import from Legacy Import there.' },
];
    ...
    <div className="settings-page">
      ...
      <div className="settings-layout">
        ...
        <div className="settings-content">
          ...
          {activeTab === 'webhooks' && <WebhookTriggersTab />}
...
const getApiUrl = () => {
  ...
  if (host === 'askalf.org' || host.endsWith('.askalf.org')) return '';
...
function IntegrationsTab() {
  ...
  const envBoundSet = new Set(available.filter(p => (p as unknown as Record<string, unknown>)['env_bound']).map(p => p.provider));
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <div className="settings-intg-provider-action">
                        ...
                        ) : envBoundSet.has(p.id) ? (
                          <span className="settings-intg-badge connected">Connected (env)</span>
                        ) : isOAuth && isAvailable ? (
                          <a
...
function WebhookTriggersTab() {
  const [triggers, setTriggers] = useState<WebhookTrigger[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [formName, setFormName] = useState('');
  const [formEvent, setFormEvent] = useState('github_push');
  ...
  const loadTriggers = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/api/v1/forge/webhook-triggers`, { credentials: 'include' });
      if (res.ok) { const data = await res.json() as { triggers: WebhookTrigger[] }; setTriggers(data.triggers || []); }
    } catch { /* ignore */ }
    setLoading(false);
  }, []);
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch(`${API_BASE}/api/v1/forge/webhook-triggers`, {
        method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: formName, event_type: formEvent, agent_name: formAgent || undefined }),
      });
      if (res.ok) { setShowCreate(false); setFormName(''); await loadTriggers(); }
    } catch { /* ignore */ }
    setCreating(false);
  };

  const handleDelete = async (id: string) => {
    await fetch(`${API_BASE}/api/v1/forge/webhook-triggers/${id}`, { method: 'DELETE', credentials: 'include' });
    await loadTriggers();
  };

  const copyUrl = (id: string) => {
    const url = `${window.location.origin}/api/v1/forge/webhooks/trigger/${id}`;
  ...
  return (
    <div className="settings-section">
      <div className="settings-section-header">
        ...
        <p className="settings-section-desc">External events that fire agent executions. Connect GitHub, Discord, or any webhook source.</p>
      </div>

      {loading ? <div style={{ padding: 20, opacity: 0.5 }}>Loading...</div> : (
        <>
          {triggers.map(t => (
            <div key={t.id} style={{ padding: '16px', margin: '12px 0', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontWeight: 600 }}>{t.name}</div>
                  <div style={{ fontSize: '0.8rem', opacity: 0.5, marginTop: 4 }}>
                    {t.event_type} {t.agent_name ? `\u2192 ${t.agent_name}` : '(auto-route)'} | {t.execution_count} fires {t.last_fired_at ? `| last: ${relativeTime(t.last_fired_at)}` : ''}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => copyUrl(t.id)} style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.1)', color: '#a5a8ff', cursor: 'pointer', fontSize: '0.8rem' }}>
                    {copiedId === t.id ? 'Copied!' : 'Copy URL'}
                  </button>
                  <button onClick={() => handleDelete(t.id)} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.1)', color: '#ef4444', cursor: 'pointer', fontSize: '0.8rem' }}>Delete</button>
                </div>
              </div>
            </div>
          ))}
          ...
          {!showCreate ? (
            <button onClick={() => setShowCreate(true)} style={{ marginTop: 16, padding: '10px 20px', borderRadius: 8, border: '1px solid rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.1)', color: '#a5a8ff', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600 }}>+ Create Webhook Trigger</button>
          ) : (
            <div style={{ padding: 20, margin: '16px 0', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 12 }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '1rem' }}>New Trigger</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <input value={formName} onChange={e => setFormName(e.target.value)} placeholder="My GitHub Trigger" style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: 'inherit' }} />
                <select value={formEvent} onChange={e => setFormEvent(e.target.value)} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'inherit' }}>
                  {eventTypes.map(e => <option key={e.value} value={e.value}>{e.label}</option>)}
                </select>
                <input value={formAgent} onChange={e => setFormAgent(e.target.value)} placeholder="Agent name (leave empty for auto-route)" style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: 'inherit' }} />
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={handleCreate} disabled={creating || !formName.trim()} style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: '#7c3aed', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>{creating ? 'Creating...' : 'Create'}</button>
                  <button onClick={() => setShowCreate(false)} style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'inherit', cursor: 'pointer' }}>Cancel</button>
                </div>
              </div>
            </div>
          )}
...
interface TeamMember { user_id: string; role: string; email: string; name: string; display_name: string; created_at: string; }
interface TeamInvite { id: string; email: string; role: string; status: string; expires_at: string; created_at: string; }
...
function TeamManagementTab() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [mRes, iRes] = await Promise.all([
        fetch(`${API_BASE}/api/v1/forge/team/members`, { credentials: 'include' }),
        fetch(`${API_BASE}/api/v1/forge/team/invites`, { credentials: 'include' }),
      ]);
      if (mRes.ok) { const d = await mRes.json() as { members: TeamMember[] }; setMembers(d.members || []); }
      if (iRes.ok) { const d = await iRes.json() as { invites: TeamInvite[] }; setInvites(d.invites || []); }
    } catch { /* ignore */ }
    setLoading(false);
  }, []);
  ...
  const handleInvite = async () => {
    ...
    setSending(true);
    try {
      const res = await fetch(`${API_BASE}/api/v1/forge/team/invite`, {
        method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: inviteEmail, role: inviteRole }),
      });
      if (res.ok) { setShowInvite(false); setInviteEmail(''); await loadData(); }
    } catch { /* ignore */ }
    setSending(false);
  };

  const handleRemove = async (userId: string) => {
    await fetch(`${API_BASE}/api/v1/forge/team/members/${userId}`, { method: 'DELETE', credentials: 'include' });
    await loadData();
  };

  const roleColors: Record<string, string> = { owner: '#7c3aed', admin: '#3b82f6', member: '#22c55e', viewer: '#6b7280' };

  return (
    <div className="settings-section">
      <div className="settings-section-header">
        <h2>Team</h2>
        <p className="settings-section-desc">Manage workspace members and invitations.</p>
      </div>

      {loading ? <div style={{ padding: 20, opacity: 0.5 }}>Loading...</div> : (
        <>
          <h3 style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 12 }}>Members ({members.length})</h3>
          {members.map(m => (
            <div key={m.user_id} style={{ padding: '12px 16px', margin: '8px 0', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <span style={{ fontWeight: 600 }}>{m.display_name || m.name || m.email}</span>
                <span style={{ marginLeft: 8, padding: '2px 8px', borderRadius: 8, fontSize: '0.7rem', fontWeight: 600, background: `${roleColors[m.role] || '#6b7280'}20`, color: roleColors[m.role] || '#6b7280' }}>{m.role}</span>
                {m.email && <span style={{ marginLeft: 8, fontSize: '0.8rem', opacity: 0.5 }}>{m.email}</span>}
              </div>
              {m.role !== 'owner' && (
                <button onClick={() => handleRemove(m.user_id)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.1)', color: '#ef4444', cursor: 'pointer', fontSize: '0.75rem' }}>Remove</button>
              )}
            </div>
          ))}
          ...
          {invites.length > 0 && (
            <>
              <h3 style={{ fontSize: '0.9rem', fontWeight: 600, margin: '20px 0 12px' }}>Pending Invites</h3>
              {invites.map(i => (
                <div key={i.id} style={{ padding: '10px 16px', margin: '6px 0', background: 'rgba(255,255,255,0.01)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 8, fontSize: '0.85rem', display: 'flex', justifyContent: 'space-between' }}>
                  <span>{i.email} <span style={{ opacity: 0.5 }}>({i.role})</span></span>
                  <span style={{ opacity: 0.4 }}>expires {relativeTime(i.expires_at)}</span>
                </div>
              ))}
            </>
          )}
          ...
          {!showInvite ? (
            <button onClick={() => setShowInvite(true)} style={{ marginTop: 16, padding: '10px 20px', borderRadius: 8, border: '1px solid rgba(99,102,241,0.3)', background: 'rgba(99,102,241,0.1)', color: '#a5a8ff', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600 }}>+ Invite Member</button>
          ) : (
            <div style={{ padding: 20, margin: '16px 0', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 12 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                <input value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="email@example.com" style={{ flex: 1, padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.03)', color: 'inherit' }} />
                <select value={inviteRole} onChange={e => setInviteRole(e.target.value)} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'inherit' }}>
                  <option value="admin">Admin</option>
                  <option value="member">Member</option>
                  <option value="viewer">Viewer</option>
                </select>
                <button onClick={handleInvite} disabled={sending || !inviteEmail.trim()} style={{ padding: '8px 16px', borderRadius: 6, border: 'none', background: '#7c3aed', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>{sending ? '...' : 'Invite'}</button>
                <button onClick={() => setShowInvite(false)} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'inherit', cursor: 'pointer' }}>Cancel</button>
              </div>
            </div>
          )}
        </>
...
function MigrationTab() {
  ...
  const handlePathLoad = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch(`${API_BASE}/api/v1/admin/migrate/legacy-import/read-config`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch(`${API_BASE}/api/v1/admin/migrate/legacy-import`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      setResult(data);
      if (data.success) {
        setMessage({ type: 'success', text: 'Legacy Import configuration imported successfully.' });
      } else {
  ...
  return (
    <div className="settings-section">
      <h2>Migration</h2>
      <p className="settings-section-desc">
        Import your Legacy Import configuration into AskAlf. Upload your <code>legacy-import.json</code> or provide its file path to migrate agents, channels, skills, and gateway settings.
      </p>
          ...
          {inputMode === 'upload' ? (
            <div className="migration-upload-zone">
              <label className="migration-file-label">
                ...
                <span>Choose <code>legacy-import.json</code> or drop it here</span>
                <input
                  type="file"
            ...
            <div className="migration-path-input">
              <div className="settings-field">
                <label>Path to legacy-import.json</label>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input
                    type="text"
                    placeholder="~/.legacy-import/legacy-import.json"
                    value={filePath}
