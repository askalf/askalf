...
// Inline stubs — hub/ components were removed during SaaS cleanup
// These render the data directly from the hub store
const InterventionGateway = lazy(() => Promise.resolve({ default: () => {
  const { interventions } = useHubStore();
  return <div style={{ padding: '1rem' }}>{interventions.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>No pending approvals</p> : interventions.map((i: any, idx: number) => <div key={idx} style={{ padding: '8px 0', borderBottom: '1px solid var(--border)' }}>{String(i.description || i.title || 'Intervention')}</div>)}</div>;
}}));
const Tickets = lazy(() => Promise.resolve({ default: () => {
  const { tickets } = useHubStore();
  return <div style={{ padding: '1rem' }}>{tickets.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>No open tickets</p> : tickets.map((t: any, idx: number) => <div key={idx} style={{ padding: '8px 0', borderBottom: '1px solid var(--border)' }}><strong>{String(t.title || '')}</strong> — {String(t.status || '')}</div>)}</div>;
}}));
const ContentFeed = lazy(() => Promise.resolve({ default: () => <div style={{ padding: '1rem', color: 'var(--text-muted)' }}>Content feed</div> }));
...
export default function OperationsTab() {
  ...
  const stats = useMemo(() => {
    ...
    const criticalTickets = tickets.filter((t) => t.priority === 'urgent' && (t.status === 'open' || t.status === 'in_progress')).length;
