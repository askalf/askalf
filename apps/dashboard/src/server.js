...
// Initialize database
const databaseUrl = process.env['DATABASE_URL'] ?? 'postgresql://askalf:askalf_dev@localhost:5432/askalf';
...
fastify.get('/api/v1/admin/projects', { config: { rateLimit: { max: 30, timeWindow: '1 minute' } } }, async (request, reply) => { // lgtm[js/missing-rate-limiting] — rate limited via fastify plugin config
  ...
  // Only show user repos — not the platform's internal codebase
...
fastify.post('/api/v1/admin/projects/create', { config: { rateLimit: { max: 5, timeWindow: '1 minute' } } }, async (request, reply) => { // lgtm[js/missing-rate-limiting]
  const user = await getAdminUser(); if (!user) return reply.code(401).send({ error: 'Not authenticated' });
...
fastify.post('/api/v1/admin/projects/clone', { config: { rateLimit: { max: 3, timeWindow: '1 minute' } } }, async (request, reply) => { // lgtm[js/missing-rate-limiting]
  const user = await getAdminUser(); if (!user) return reply.code(401).send({ error: 'Not authenticated' });
...
// Read an legacy.json from a server-side path
// ============================================
...
fastify.post('/api/v1/admin/migrate/legacy/read-config', { config: { rateLimit: { max: 10, timeWindow: '1 minute' } } }, async (request, reply) => { // rate-limited
  const user = await getAdminUser();
  if (!user) return reply.code(401).send({ error: 'Not authenticated' });
...
// Import legacy.json configuration
fastify.post('/api/v1/admin/migrate/legacy', async (request, reply) => {
  const user = await getAdminUser();
  if (!user) return reply.code(401).send({ error: 'Not authenticated' });
  ...
  try {
    ...
    // 1) Import agents — handle both flat array and Legacy's agents.list format
    ...
    // Model mapping: Legacy models → AskAlf equivalents
    const MODEL_MAP = {
    ...
    for (const agent of agentsList) {
      try {
        ...
        const name = agent.name || agent.id || 'legacy-agent';
        const slug = name.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-');
        const rawModel = agent.model || defaultModel;
        const model = MODEL_MAP[rawModel] || rawModel;
        const description = agent.description || `Migrated from Legacy`;
        ...
        `, [
          ...
          JSON.stringify({ source: 'legacy-migration', original_id: agent.id || null }),
        ]);
        agentsImported++;
      } catch (err) {
    ...
    if (config.gateway) {
      try {
        ...
        const settings = {
          legacy_gateway_url: gwUrl,
          legacy_gateway_token: gwToken ? encryptApiKey(gwToken) : null,
    ...
    console.error('[Migration] Legacy import error:', err);
