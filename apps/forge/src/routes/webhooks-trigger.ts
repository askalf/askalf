...
export async function webhookTriggerRoutes(app: FastifyInstance): Promise<void> {
   ...
   */
  app.post(
    '/api/v1/forge/webhooks/trigger/:id',
   ...
   */
  app.get('/api/v1/forge/webhook-triggers', { preHandler: [authMiddleware] }, async () => {
    const triggers = await query<TriggerConfig>(`SELECT * FROM webhook_triggers ORDER BY created_at DESC`);
    return { triggers };
  });

  app.post('/api/v1/forge/webhook-triggers', { preHandler: [authMiddleware] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.delete('/api/v1/forge/webhook-triggers/:id', { preHandler: [authMiddleware] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as { id: string };
