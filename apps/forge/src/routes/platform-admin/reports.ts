    ...
    async () => {
      ...
      return getCached('reports:metrics', 30, async () => {
        const [
          ...
          queryOne<{ count: string }>('SELECT COUNT(*) as count FROM agent_tickets WHERE deleted_at IS NULL'),
          queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM agent_tickets WHERE deleted_at IS NULL AND status IN ('open', 'in_progress')`),
        ]);
