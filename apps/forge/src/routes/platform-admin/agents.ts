...
export async function registerAgentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const [agents, agentTotal, execStats, runningExecs, interventionCounts, schedules] = await Promise.all([
        ...
        query<{ agent_id: string; count: string }>(
          `SELECT agent_id, COUNT(*)::text as count FROM agent_interventions WHERE status = 'pending' GROUP BY agent_id`
        ),
        query<{ agent_id: string; schedule_type: string; schedule_interval_minutes: number | null }>(
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { id } = request.params as { id: string };
      const body = request.body as Record<string, unknown> || {};
      const agent = await queryOne<{ id: string; name: string; system_prompt: string; model_id: string | null; max_cost_per_execution: number | null; max_iterations: number | null; schedule_interval_minutes: number | null }>(
        'SELECT id, name, system_prompt, model_id, max_cost_per_execution, max_iterations, schedule_interval_minutes FROM forge_agents WHERE id = $1', [id]
      );
      if (!agent) return reply.code(404).send({ error: 'Agent not found' });
      ...
      const execId = ulid();
      const ownerId = (request as unknown as Record<string, unknown>)["userId"] as string || 'admin';
      await query(
        `INSERT INTO forge_executions (id, agent_id, owner_id, status, input, started_at) VALUES ($1, $2, $3, 'pending', $4, NOW())`,
        [execId, id, ownerId, prompt]
      );
      runDirectCliExecution(execId, id, prompt, ownerId, {
        systemPrompt: agent.system_prompt,
        modelId: agent.model_id ?? undefined,
        maxBudgetUsd: agent.max_cost_per_execution ? String(agent.max_cost_per_execution) : undefined,
        maxTurns: agent.max_iterations ?? undefined,
        scheduleIntervalMinutes: agent.schedule_interval_minutes ?? undefined,
      }).catch(err => {
        const msg = err instanceof Error ? err.message : String(err);
        query(`UPDATE forge_executions SET status = 'failed', error = $1, completed_at = NOW() WHERE id = $2`, [msg.substring(0, 2000), execId]).catch(() => {});
      });
  ...
  app.post(
    ...
    async (request: FastifyRequest, _reply: FastifyReply) => {
      ...
      for (const agent of agents) {
        try {
          const execId = ulid();
          const batchOwnerId = (request as unknown as Record<string, unknown>)["userId"] as string || 'admin';
          await query(
            `INSERT INTO forge_executions (id, agent_id, owner_id, status, input, started_at) VALUES ($1, $2, $3, 'pending', 'Scheduled batch execution', NOW())`,
            [execId, agent.id, batchOwnerId]
          );
          void runDirectCliExecution(execId, agent.id, 'Scheduled batch execution', batchOwnerId);
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const taskOwnerId = (request as unknown as Record<string, unknown>)["userId"] as string || 'admin';
      await query(
        `INSERT INTO forge_executions (id, agent_id, owner_id, status, input, started_at) VALUES ($1, $2, $3, 'pending', $4, NOW())`,
        [execId, id, taskOwnerId, input]
      );
      void runDirectCliExecution(execId, id, input, taskOwnerId);
