...
export async function tenantRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (request: FastifyRequest) => {
      ...
      const tenants = await substrateQuery<Record<string, unknown>>(
        `SELECT t.*, tm.role
         FROM tenants t
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const countResult = await substrateQueryOne<{ count: string }>(
  ...
  app.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Verify ownership
      const member = await substrateQueryOne<{ role: string }>(
      ...
      const tenant = await substrateQueryOne(
  ...
  app.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      await substrateQuery(`UPDATE tenants SET status = 'archived', updated_at = NOW() WHERE id = $1`, [id]);
