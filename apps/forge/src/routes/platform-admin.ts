 ...
 * schedules, audit, and orchestration stats. Queries both Forge and Platform DBs directly.
 */
...
export async function platformAdminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (_request: FastifyRequest, _reply: FastifyReply) => {
      ...
      const interventionCounts = await query<{ agent_id: string; count: string }>(
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const [agents, executions, pendingCount] = await Promise.all([
        ...
        queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM agent_interventions WHERE status = 'pending'`),
      ]);
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const oldIntervention = await queryOne<{
  ...
  app.patch(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const oldTicket = await queryOne<Record<string, unknown>>(
  ...
  app.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const old = await queryOne<Record<string, unknown>>(
  ...
  app.get(
    ...
    async () => {
      const [agents, schedules] = await Promise.all([
        ...
        query<Record<string, unknown>>('SELECT * FROM agent_schedules'),
      ]);
  ...
  app.get(
    ...
    async (request: FastifyRequest) => {
      ...
      const [entries, countResult] = await Promise.all([
        ...
        queryOne<{ total: number }>(`SELECT COUNT(*)::int as total FROM agent_audit_log ${where}`, params),
      ]);
  ...
  app.get(
    ...
    async () => {
      const [
        ...
        queryOne<{ count: string }>('SELECT COUNT(*) as count FROM agent_tickets'),
        queryOne<{ count: string }>(`SELECT COUNT(*) as count FROM agent_tickets WHERE status IN ('open', 'in_progress')`),
      ]);
  ...
  app.get(
    ...
    async (request: FastifyRequest) => {
      ...
      const [items, countResult] = await Promise.all([
        ...
        queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_findings ${where}`, params),
      ]);
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { id } = request.params as { id: string };
      const finding = await queryOne<Record<string, unknown>>(
  ...
  app.get(
    ...
    async () => {
      const [userCount, agentCount] = await Promise.all([
        queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM users`).catch(() => ({ count: '0' })),
  ...
  app.get(
    ...
    async (request: FastifyRequest) => {
      ...
      const [users, countResult] = await Promise.all([
        ...
        queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM users u ${where}`, params),
      ]);
  ...
  app.get(
    ...
    async () => {
      const [total, active, suspended, today] = await Promise.all([
        ...
        queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM users WHERE status = 'suspended'`),
        queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM users WHERE created_at > NOW() - INTERVAL '24 hours'`),
      ]);
      return {
        users: {
...
async function runSchedulerTick(): Promise<void> {
  ...
  try {
    ...
    const dueAgents = await query<Record<string, unknown>>(
    ...
    if (dueAgents.length === 0) {
      if (tickCount % 5 === 0) {
        const nextDue = await queryOne<{ next: string }>(`SELECT MIN(next_run_at) as next FROM agent_schedules`);
