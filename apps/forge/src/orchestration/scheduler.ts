...
import { InMemoryQueue, InMemoryWorker } from '@askalf/redis-adapter';
import type { Job as MemoryJob } from '@askalf/redis-adapter';
...
export interface WorkflowJobData extends Record<string, unknown> {
  runId: string;
  workflowId: string;
  ownerId: string;
}
...
export class ForgeScheduler {
  private readonly queue: Queue<WorkflowJobData> | InMemoryQueue<WorkflowJobData>;
  private worker: Worker<WorkflowJobData> | InMemoryWorker<WorkflowJobData> | null = null;
  ...
  constructor(
    ...
    this.standalone = process.env['ASKALF_MODE'] === 'standalone';

    if (this.standalone) {
      this.queue = new InMemoryQueue<WorkflowJobData>('workflow-runs');
    } else {
  ...
  /**
   * Start a Worker that processes enqueued workflow run jobs.
   * Uses BullMQ in Docker mode, InMemoryWorker in standalone mode.
   */
  processWorkflowJobs(
    redisUrl: string,
    concurrency: number = 5,
  ): Worker<WorkflowJobData> | InMemoryWorker<WorkflowJobData> {
    if (this.standalone) {
      const memWorker = new InMemoryWorker<WorkflowJobData>(
        'workflow-runs',
        async (job: MemoryJob<WorkflowJobData>) => {
          await this.executeWorkflowRun(job.data);
        },
        { concurrency },
      );
      memWorker.attachQueue(this.queue as InMemoryQueue<WorkflowJobData>);
      this.worker = memWorker;

      memWorker.on('failed', (job: MemoryJob<WorkflowJobData>, err: unknown) => {
        const errMsg = err instanceof Error ? err.message : String(err);
        logger.error(`[ForgeScheduler] Job ${job?.id ?? 'unknown'} failed: ${errMsg}`);
        if (job?.data?.runId) {
          import('../database.js').then(({ query: dbQuery }) =>
            dbQuery(
              ...
              [errMsg.substring(0, 2000), job.data.runId],
            ),
