...
async function fetchActiveChannels(): Promise<ChannelConfig[]> {
  try {
    return await query<ChannelConfig>(
...
async function resolveAlertAssignment(metric: string): Promise<string> {
  ...
  for (const type of types) {
    const match = await query<{ name: string }>(
  ...
  const fallback = await query<{ name: string }>(
    `SELECT name FROM forge_agents
     WHERE status = 'active'
       AND (is_decommissioned IS NULL OR is_decommissioned = false)
       AND (is_internal IS NULL OR is_internal = false)
...
async function createAlertTicket(alert: Alert): Promise<void> {
  try {
    ...
    if (alert.metric === 'hourly_cost') {
      const costSpikeTicket = await query<{ id: string }>(
        `SELECT id FROM agent_tickets
...
export async function runHealthCheck(): Promise<HealthReport> {
  ...
  // Note: interventions live in the askalf DB (agent_interventions), not forge DB.
