...
export class AgentCommunication {
  ...
  constructor(urlOrPubOrAdapter: string | Redis | IRedisAdapter, subClient?: Redis) {
    if (typeof urlOrPubOrAdapter === 'string') {
      this.pub = new Redis(urlOrPubOrAdapter, { maxRetriesPerRequest: null });
      this.sub = new Redis(urlOrPubOrAdapter, { maxRetriesPerRequest: null });
    } else if (urlOrPubOrAdapter instanceof Redis) {
      this.pub = urlOrPubOrAdapter;
      this.sub = subClient!;
    } else {
      // IRedisAdapter path
      this.pub = urlOrPubOrAdapter;
      this.sub = urlOrPubOrAdapter.duplicate();
    }
...
export function initAgentCommunication(adapter: IRedisAdapter): AgentCommunication;
export function initAgentCommunication(urlOrAdapter: string | IRedisAdapter): AgentCommunication {
  if (instance) return instance;
  if (typeof urlOrAdapter === 'string') {
    instance = new AgentCommunication(urlOrAdapter);
  } else {
    instance = new AgentCommunication(urlOrAdapter);
  }
