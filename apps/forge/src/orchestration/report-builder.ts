...
export async function generateReport(type: 'daily' | 'weekly', sections: string[]): Promise<GeneratedReport> {
  ...
  const [
    ...
    Promise.all([
      queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_tickets WHERE status IN ('open','in_progress') AND deleted_at IS NULL`),
      queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_tickets WHERE status = 'closed' AND updated_at > NOW() - INTERVAL '${interval}'`),
      queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_tickets WHERE created_at > NOW() - INTERVAL '${interval}' AND deleted_at IS NULL`),
    ]),
    Promise.all([
      queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_findings WHERE severity = 'critical' AND created_at > NOW() - INTERVAL '${interval}'`).catch(() => ({ count: '0' })),
      queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_findings WHERE severity = 'warning' AND created_at > NOW() - INTERVAL '${interval}'`).catch(() => ({ count: '0' })),
      queryOne<{ count: string }>(`SELECT COUNT(*)::text as count FROM agent_findings WHERE severity = 'info' AND created_at > NOW() - INTERVAL '${interval}'`).catch(() => ({ count: '0' })),
    ]),
    ...
    query<Record<string, unknown>>(`SELECT * FROM agent_findings WHERE created_at > NOW() - INTERVAL '${interval}' ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'warning' THEN 1 ELSE 2 END, created_at DESC LIMIT 10`).catch(() => []),
  ]);
