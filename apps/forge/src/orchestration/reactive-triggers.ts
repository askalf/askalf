...
async function handleExecutionCompletedInner(event: ExecutionEvent): Promise<void> {
  ...
  for (const signal of foreignSignals) {
    ...
    // Dedup: skip if target agent already has an open ticket from this same source execution
    ...
    if (existingTicket.length > 0) {
      console.log(
        `[ReactiveTrigger] Skipping — ${target.agent_name} already has open ticket ${existingTicket[0]!.id} (dedup)`,
      );
      continue;
    }
