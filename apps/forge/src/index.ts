...
import { federationHandshakeRoutes } from './routes/federation-handshake.js';
...
import { createAdapterFromEnv } from '@askalf/database-adapter';
import { createRedisAdapterFromEnv } from '@askalf/redis-adapter';
...
if (marketplaceSyncRoutes) await marketplaceSyncRoutes(app);
...
await webhookTriggerRoutes(app);
await demoRoutes(app);
await incidentRoutes(app);
await federationHandshakeRoutes(app);
...
async function start(): Promise<void> {
  ...
  const standaloneMode = process.env['ASKALF_MODE'] === 'standalone';

  if (standaloneMode) {
    const dbAdapter = await createAdapterFromEnv();
    initializeDatabaseWithAdapter(dbAdapter);
    logger.info('[Forge] PGlite database initialized (standalone mode)');
  } else {
    ...
    if (config.databaseUrl) {
      initializeDatabase(config.databaseUrl);
      logger.info('[Forge] AskAlf database connection initialized');
    }
  }
  ...
  try {
    ...
    if (standaloneMode) {
      const memRedis = createRedisAdapterFromEnv();
      await initMemoryManager(memRedis);
    } else {
    ...
    // Initialize event bus, shared context, and communication
    if (standaloneMode) {
      const memRedis = createRedisAdapterFromEnv();
      await initEventBus(memRedis);
      initSharedContext(memRedis);
      initAgentCommunication(memRedis);
      logger.info('[Forge] Standalone mode: event bus + shared context + communication (in-memory)');
    } else {
    ...
    if (standaloneMode) {
      const memRedis = createRedisAdapterFromEnv();
      initRateLimit(memRedis, internalKeys.map((k) => k.key_prefix));
      logger.info('[Forge] Rate limiter initialized (in-memory)');
    } else {
