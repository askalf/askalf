...
export class UnifiedDispatcher {
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      // Immune System — check for recurring failures and create incidents (every 30 ticks = ~15 min)
      if (this.tickCount % 30 === 0) {
        try {
          const { checkAntibody, createIncident } = await import('../orchestration/immune-system.js');
          // Check for repeated failures in the last 30 min
          const failures = await query<{ agent_name: string; error: string; cnt: string }>(
            `SELECT a.name as agent_name, LEFT(e.error, 100) as error, COUNT(*)::text as cnt
             FROM forge_executions e JOIN forge_agents a ON a.id = e.agent_id
             WHERE e.status = 'failed' AND e.created_at > NOW() - INTERVAL '30 minutes'
             GROUP BY a.name, LEFT(e.error, 100) HAVING COUNT(*) >= 2`,
          ).catch(() => []);

          for (const f of failures) {
            const antibody = await checkAntibody(f.error || f.agent_name);
            if (antibody.exists) {
              console.log(`[ImmuneSystem] Antibody matched: ${f.error?.substring(0, 50)} → ${antibody.resolution?.substring(0, 50)}`);
            } else {
              await createIncident(
                `${f.agent_name} failing repeatedly: ${f.error?.substring(0, 60)}`,
                parseInt(f.cnt) >= 4 ? 'high' : 'medium',
                'Dispatcher', f.error || 'Unknown failure',
              );
            }
          }
        } catch (err) {
          console.warn(`[Dispatcher] Immune system error: ${err instanceof Error ? err.message : String(err)}`);
        }
      }

      // Natural Selection — evaluate underperformers every 480 ticks (~4 hours)
      if (this.tickCount % 480 === 0) {
        try {
          const { runNaturalSelection } = await import('../orchestration/natural-selection.js');
          await runNaturalSelection();
        } catch (err) {
          console.warn(`[Dispatcher] Natural selection error: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
...
Investigate and take appropriate action based on your system prompt.${fleetContext}`;
  ...
  private async processScheduledAgents(
    ...
    for (const agent of agents) {
      ...
      const isScheduledContent = agent.type === 'content' || agent.type === 'custom';
      ...
      // Ticket-gate: skip workers with no tickets (monitors and content agents run on schedule regardless)
      if (assignedTickets.length === 0 && !isMonitor && !isScheduledContent) {
        await this.advanceSchedule(agent);
      ...
      if (isMonitor) {
        ...
        // TICKET STORM PREVENTION: check total open tickets before allowing creation
        const totalOpenTickets = await query<{ count: string }>(
          `SELECT COUNT(*)::text as count FROM agent_tickets WHERE status IN ('open','in_progress') AND deleted_at IS NULL`,
        ).catch(() => [{ count: '0' }]);
        const openCount = parseInt(totalOpenTickets[0]?.count || '0');
        const ticketCreationAllowed = openCount < 5; // Hard cap: max 5 open tickets fleet-wide
        ...
        const ticketRule = ticketCreationAllowed
          ? '3. TICKETS: If you find a NEW issue that has no existing ticket, create ONE ticket max. Assign to Builder.'
          : '3. TICKETS: There are already 5+ open tickets. Do NOT create any new tickets. Only create findings.';

        const input = `[PATROL CYCLE — ${new Date().toISOString()}] You are ${agent.name}.

You are a MONITOR agent. Patrol the system, detect issues, report findings.${ownTicketBlock}
...
${ticketRule}
4. SUMMARY: Create one summary finding with what you checked and found.

HARD RULES:
- Do NOT fix issues yourself — only detect and report.
- Do NOT create duplicate tickets.
- Do NOT create tickets about other agents' performance or pending tickets.
- Maximum 1 new ticket per patrol cycle. Zero if 5+ are already open.
...
PATROL. Detect. Report. Stop.${fleetContext}`;
      ...
      } else if (isScheduledContent) {
        // Content/Custom agents: run on schedule with their OWN system prompt — no patrol template
        const input = `[SCHEDULED CYCLE — ${new Date().toISOString()}] You are ${agent.name}.

Execute your role as defined in your system prompt. This is a scheduled run.

RULES:
- Do your actual job — not patrol or monitoring.
- Search memory first for context from your last run.
- Store results in memory when done.
- If you find something that needs Builder attention, create ONE ticket max.
...
${fleetContext}`;
        ...
        // COST CONTROL: max 1 ticket per dispatch cycle per worker to prevent cost spikes
        const ticketsToDispatch = unworkedTickets.slice(0, Math.min(slotsAvailable, 1));
...
FOCUS. Work the ticket. Ship code. Stop.${fleetContext}`;
  ...
  private async checkTenantBudget(tenantId: string): Promise<{ period: string; spent: number; limit: number } | null> {
    try {
      ...
      const tenant = await queryOne<{ budget_limit_daily: string | null; budget_limit_monthly: string | null }>(
  ...
  private async buildFleetContext(): Promise<string> {
    ...
    // Build signal awareness from the nervous system
    let signalContext = '';
    try {
      const { buildFleetAwareness } = await import('../orchestration/nervous-system.js');
      signalContext = buildFleetAwareness();
    } catch { /* non-critical */ }

    // Build knowledge context
    let knowledgeStats = '';
    try {
      const { getGraphStats } = await import('../orchestration/collective-memory.js');
