...
let rateLimitAdapter: IRedisAdapter | null = null;
...
export function initRateLimit(adapter: IRedisAdapter, keyPrefixes?: string[]): void;
export function initRateLimit(urlOrAdapter: string | IRedisAdapter, keyPrefixes: string[] = []): void {
  for (const p of keyPrefixes) internalKeyPrefixes.add(p);

  if (typeof urlOrAdapter === 'string') {
    rateLimitRedis = new Redis(urlOrAdapter, {
...
export async function closeRateLimitRedis(): Promise<void> {
  ...
  if (rateLimitAdapter) {
    await rateLimitAdapter.quit().catch((e) => { if (e) console.debug("[catch]", String(e)); });
...
export async function cacheKeyRateLimit(tokenHash: string, limit: number): Promise<void> {
  const rl = rateLimitRedis || rateLimitAdapter;
  if (!rl) return;
  try {
    await rl.setex(`rl:tc:${tokenHash}`, TIER_CACHE_TTL, String(limit));
  } catch {
...
export async function rateLimitHook(
  ...
  const rl = rateLimitRedis || rateLimitAdapter;
  if (!rl) return; // Fail open if not initialized
  ...
  if (isAuthed && tokenHash) {
    try {
      const cached = await rl.get(`rl:tc:${tokenHash}`);
      if (cached !== null) {
  ...
  const now = Date.now();
  const uid = `${now}:${randomBytes(6).toString('hex')}`;
