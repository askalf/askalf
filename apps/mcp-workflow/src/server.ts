...
/**
 * MCP Workflow Server (port 3010)
 *
 * Exposes workflow management tools via MCP protocol:
 * - ticket_ops: create, update, assign, list, get, add_note, audit_history
 * - finding_ops: create, list, get, promote
 * - intervention_ops: create, list, get, check
 * - agent_call: delegate tasks to other agents (via HTTP to forge)
 *
 * All tools write to the substrate database (agent_tickets, agent_findings,
 * agent_interventions, agent_audit_log).
 */
...
const log = (msg: string) => console.log(`[mcp-workflow] ${new Date().toISOString()} ${msg}`);
...
app.get('/health', (_req, res) => {
  res.json({ status: 'healthy', service: 'mcp-workflow' });
});
...
async function handleFindingOps(args: Record<string, unknown>): Promise<string> {
  ...
  switch (action) {
    case 'create': {
      ...
      // Auto-store finding as semantic memory in fleet brain (mirrors worker.ts behavior)
      try {
        ...
        await forgeQuery(
          ...
          [
            memId,
            (args['agent_id'] as string) ?? 'unknown',
            finding,
            importanceMap[severity] ?? 0.4,
            JSON.stringify({ source_type: 'finding', category, severity, agent_name: agentName, finding_id: id }),
          ],
        );
      } catch (memErr) {
        // Non-fatal — don't fail the finding creation
        log(`Semantic memory auto-store failed: ${memErr instanceof Error ? memErr.message : memErr}`);
      }
    ...
    case 'create': {
      ...
      const result = await p.query(
        ...
        [
          ...
          (args['agent_id'] as string) ?? 'unknown',
          agentName,
          (args['agent_type'] as string) ?? 'custom',
          (args['task_id'] as string) ?? null,
          (args['type'] as string) ?? 'feedback',
          title,
          (args['description'] as string) ?? null,
          (args['context'] as string) ?? null,
          (args['proposed_action'] as string) ?? null,
        ],
      );
...
function createMCPServer(): Server {
  const server = new Server(
    { name: 'mcp-workflow', version: '1.0.0' },
    { capabilities: { tools: {} } },
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        case 'ticket_ops':
          result = await handleTicketOps(args as Record<string, unknown>);
          break;
        case 'finding_ops':
          result = await handleFindingOps(args as Record<string, unknown>);
          break;
        case 'intervention_ops':
          result = await handleInterventionOps(args as Record<string, unknown>);
          break;
        case 'agent_call':
          result = await handleAgentCall(args as Record<string, unknown>);
          break;
        default:
...
app.listen(PORT, '0.0.0.0', () => {
  log(`MCP Workflow server listening on port ${PORT}`);
