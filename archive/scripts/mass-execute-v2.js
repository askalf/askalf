// Mass Execution v2 - Extended generators
...
const G = {
  // Math
  addition: i => [`What is ${i*7+3} + ${i*3+2}?`, `${i*11+5} plus ${i*5+1}`, `add ${i*4+1} and ${i*6+2}`],
  ...
  power: i => [`${i%10+2} to the power of ${i%4+2}`, `${i%8+2} ^ ${i%3+2}`],
  ...
  // Temperature - fixed format for shards
  celsiusToF: i => [`Convert ${i*5} celsius to fahrenheit`, `${i*3+10}°C to fahrenheit`, `${i*4-5} celsius to fahrenheit`],
  fahrenheitToC: i => [`${i*9+32} fahrenheit`, `${i*7+50}°F`, `${i*5+40}f`],  // No "Convert" - shard expects direct format

  // Distance
  kmToMiles: i => [`Convert ${i*10+10} km to miles`, `${i*15+5} kilometers to miles`],
  milesToKm: i => [`Convert ${i*5+5} miles to km`],
  kmToMilesAlt: i => [`convert ${i*10+10} km to miles`, `${i*15+5} km to miles`],  // For kilometers-to-miles-converter

  // String
  reverse: i => { const w=['hello','world','testing','substrate','memory','shard','crystal']; return [`Reverse "${w[i%w.length]}"`, `reverse "${w[(i+2)%w.length]}"`]; },
  uppercase: i => { const w=['hello','world','testing','substrate']; return [`Convert "${w[i%w.length]}" to uppercase`]; },
  ...
  charCount: i => [`count characters in "hello"`, `character count of "testing"`],
  slug: i => [`slugify "Hello World"`, `slugify "My Test Post"`],
  stringLength: i => { const w=['hello','testing','substrate','world']; return [`what is the length of "${w[i%w.length]}"?`, `length of "${w[(i+1)%w.length]}"`]; },

  // Number base conversion - fixed formats
  binaryToDecimal: i => { const b=['1010','1111','10000','11001','101010']; return [`Convert binary ${b[i%b.length]} to decimal`, `binary ${b[(i+1)%b.length]} to decimal`]; },
  decimalToBinary: i => [`${i*7+15} to binary`, `${i*11+20} in binary`],  // No "Convert"
  hexToDecimal: i => { const h=['FF','1A','2B','3C','4D','5E']; return [`Convert 0x${h[i%h.length]} to decimal`, `0x${h[(i+2)%h.length]} to decimal`]; },
  decimalToHex: i => [`${i*17+100} to hex`, `${i*23+50} in hex`, `${i*19+75} to hexadecimal`],  // No "Convert"

  // Validation
  email: i => [`is test${i}@example.com a valid email?`, `validate email user${i}@domain.org`],
  validateUrl: i => [`validate url https://example${i}.com`, `is https://test${i}.org a valid url`],
  validatePhone: i => [`validate phone +1-555-${100+i}-4567`, `check phone 555${i}001234`],

  // Text operations
  capitalize: i => [`capitalize "hello world"`, `capitalize "testing substrate"`, `title case "my text"`],
  trim: i => [`trim "  hello  "`, `strip "   test   "`],
  extractNumbers: i => [`extract numbers from "order ${i*100+123} has ${i+5} items"`],
  countVowels: i => { const w=['hello','testing','algorithm']; return [`count vowels in "${w[i%w.length]}"`]; },
  palindrome: i => { const w=['racecar','hello','madam','level','world']; return [`is "${w[i%w.length]}" a palindrome`]; },
  prime: i => [`is ${i*7+11} prime`, `is ${i*3+17} prime`],
  evenOdd: i => [`is ${i*5+3} even or odd`, `is ${i*2+4} even`],
  removeDupChars: i => [`remove duplicate characters from "hello"`, `remove duplicate chars from "mississippi"`],
  truncate: i => [`truncate "hello world" to 5`, `truncate "testing" to 4`],
  repeat: i => [`repeat "ab" 3 times`, `"test" 4 times`],
  replace: i => [`replace "l" with "x" in "hello"`, `replace "e" with "a" in "test"`],
  longestWord: i => { const p=['the quick brown fox','hello world test']; return [`find longest word in "${p[i%p.length]}"`, `longest word in "${p[(i+1)%p.length]}"`]; },
  ...
  // Lists
  sortList: i => [`sort ${i+5}, ${i+2}, ${i+8}, ${i+1}, ${i+9}`, `sort 5, 2, 8, 1, 9`],
  sortAsc: i => [`sort ${i+5}, ${i+2}, ${i+8}, ${i+1} ascending`, `${i*3}, ${i*5}, ${i*2}, ${i*4} ascending`],
  max: i => [`max of ${i+5}, ${i+2}, ${i+8}, ${i+1}`, `maximum in 5, 2, 8, 1, 9`],
  min: i => [`min of ${i+5}, ${i+2}, ${i+8}, ${i+1}`, `minimum in 5, 2, 8, 1, 9`],
  average: i => [`average of ${i*10}, ${i*20}, ${i*30}`, `mean of 10, 20, 30`],
  sum: i => [`sum of ${i+1}, ${i+2}, ${i+3}, ${i+4}`, `sum 1, 2, 3, 4, 5`],
  sumFromText: i => [`sum ${i+1}, ${i+2}, ${i+3}`, `${i*5}, ${i*10}, ${i*15}`],
  median: i => [`calculate median of ${i}, ${i+2}, ${i+4}`, `median of 1, 2, 3, 4, 5`],
  countItems: i => [`count items in 1, 2, 3, 4, 5`, `how many elements in 1, 2, 3`],
  unique: i => [`unique values in 1, 2, 2, 3, 3, 3`, `dedupe 5, 5, 3, 3, 1`],
  removeDuplicates: i => [`remove duplicates from 1, 2, 2, 3, 3`, `dedupe list 5, 5, 3, 3, 1`],

  // Data extraction
  extractEmail: i => [`extract email from "Contact test${i}@example.com"`, `find email in "email: user@domain.org"`],
  ...
  bmi: i => [`bmi for ${60+i}kg 1.75m`],
  leapYear: i => { const y=[2000,2004,2100,2020,1900,2024,2023,2022]; return [`Is ${y[i%y.length]} a leap year?`, `is ${y[(i+2)%y.length]} a leap year`]; },

  // Greetings/conversational
  greeting: i => { const g=['hello','hi','hey','good morning','good afternoon']; return [g[i%g.length], `${g[(i+1)%g.length]} there`]; },
  howAreYou: i => { const t=['today','this morning','this evening']; return [`how are you ${t[i%t.length]}`]; },

  // Knowledge queries
  capitalCity: i => { const c=['France','Germany','Japan','Italy','Spain','UK','USA','Canada','Brazil','India']; return [`what is the capital of ${c[i%c.length]}`]; },
  explainConcept: i => { const c=['gravity','photosynthesis','democracy','DNA','evolution']; return [`explain ${c[i%c.length]} in simple terms`]; },
  explainBasics: i => { const t=['python','javascript','cooking','chess','photography']; return [`explain the basics of ${t[i%t.length]}`]; },
  sunTemp: i => [`what is the temperature of the sun`, `temperature of the sun`],
  haiku: i => { const t=['nature','love','seasons','moon','mountains']; return [`write a haiku about ${t[i%t.length]}`]; },

  // Abstract/reframe (simple test cases)
  formalParaphrase: i => { const s=['time is money','actions speak louder than words']; return [`rephrase "${s[i%s.length]}" as a concise abstract aphorism`]; },
...
function match(name, template) {
  ...
  if (n.includes('url-encode')||n.includes('url')&&n.includes('encode')) return 'urlEncode';
  if (n.includes('url-decode')||n.includes('url')&&n.includes('decode')) return 'urlDecode';
...
async function main() {
  ...
  console.log('MASS EXECUTION v2 - Battle Testing Shards');
  console.log('='.repeat(60));

  // First unarchive the fixable shards
  await pool.query(`UPDATE procedural_shards SET lifecycle='testing', execution_count=0, success_count=0, failure_count=0 WHERE name IN ('decimal-to-hex', 'fahrenheit-to-celsius') AND lifecycle='archived'`);
