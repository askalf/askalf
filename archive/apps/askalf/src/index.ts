...
app.addHook('onRequest', async (request) => {
  httpRequestsInFlight.inc({ service: 'askalf' });
  (request as unknown as Record<string, unknown>)['_metricsStart'] = process.hrtime.bigint();
});

app.addHook('onResponse', async (request, reply) => {
  httpRequestsInFlight.dec({ service: 'askalf' });
  const start = (request as unknown as Record<string, unknown>)['_metricsStart'] as bigint | undefined;
  httpRequestsTotal.inc({ service: 'askalf', method: request.method, status: String(reply.statusCode) });
  if (start) {
    const durationMs = Number(process.hrtime.bigint() - start) / 1_000_000;
    httpRequestDuration.observe(durationMs, { service: 'askalf', method: request.method });
  }
});
